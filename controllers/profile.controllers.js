...
const FindAllProfiles = async (req ,res)=>{
    try {
       const data =  await ProfileModel.find().populate('user', ["nom","prenom" ,"email","numTelp" ,"role"])
...
const FindSingleProfile = async (req ,res)=>{
    try {
        const data =  await ProfileModel.findOne({user: req.user.id}).populate('user', ["nom", "prenom","numTelp","email", "role"])
        res.status(200).json(data)
